...
## Testing Guidelines
There is no automated test suite yet. Validate changes by running `npm run build` and reviewing the relevant generated output in `blogs/dist/`. Match verification scope to the files and behavior changed. Do not run `npm run check:ui`, a full rendered-route sweep, or unrelated screenshot checks for content-only, data-only, preprocessing, documentation, or narrowly scoped non-UI changes. When a UI change is made, test only the affected route, component, interaction, and necessary responsive viewport; prefer a targeted command such as `node scripts/check-rendered-site.js --route=<affected-route>` plus a focused manual check. Run the complete UI or rendered-site suites only when the change affects shared navigation, global layout or CSS, common UI infrastructure, many routes, or when the user explicitly requests broad regression testing. For editor changes, verify only the affected draft save/upload flow with `cd blogs && npm run edit`.
