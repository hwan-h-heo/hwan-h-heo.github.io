...
**Normalizing Flow**부터 **Continuous Normalizing Flow**, **Conditional Flow Matching**까지 살펴보고, **Rectified Flow가 왜 linear path를 채택하는지** 수학적·물리적으로 이해해본다.

Flow Matching은 이론적 깊이 때문에 쉽게 접근할 수 있는 설명 자료가 아직 부족하다. 이 글은 수학, 물리학, 최신 논문에 흩어진 핵심 아이디어를 하나의 흐름으로 엮어, “왜 직선 경로가 효과적인가?”라는 근본적인 질문에 대한 직관을 만드는 것을 목표로 한다.
