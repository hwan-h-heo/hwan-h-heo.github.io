...
<p class="lang eng"> The current implementation is maintained in <code>js/simple-model-viewer.js</code>. The snippet below is an early prototype excerpt kept for historical context, and the updated usage example right after it reflects the current API surface. </p>
<details id="div-1" class="code-disclosure">
      <summary class="code-disclosure-summary">
          <strong><em>Historical prototype excerpt</em></strong>
      </summary>
...
&lt;script type=&quot;module&quot;>
    import { SimpleModelViewer } from &#39;simple-model-viewer.js&#39;;
        ...
        viewer.addEventListener(&#39;viewer-load&#39;, (event) =&gt; {
            console.log(&#39;Loaded&#39;, event.detail.source, event.detail.meshCount);
        });

        viewer.fitCameraToModel();
&lt;/script&gt; </code></pre>
<p class="lang eng"> The current viewer also exposes a broader API surface for reuse. Beyond the original embedding options, you can now drive custom HDR backgrounds with <code>environment-url</code>, camera defaults with <code>camera-target</code> and <code>camera-up</code>, playback with <code>animation</code>, <code>animation-speed</code>, <code>animation-loop</code>, and interaction/performance behavior with <code>selection-mode</code> and <code>performance-mode</code>.</p>
<ul class='lang eng'>
    <li><strong>Useful methods</strong>: <code>loadModelFromUrl()</code>, <code>discardModel()</code>, <code>resetView()</code>, <code>fitCameraToModel()</code>, <code>selectMeshByName()</code>, <code>selectMeshByIndex()</code>, <code>clearSelection()</code>, <code>setEnvironment()</code>, <code>captureScreenshot()</code>, <code>exportState()</code>, and <code>importState()</code>.</li>
    <li><strong>Observable events</strong>: <code>viewer-load</code>, <code>viewer-error</code>, <code>viewer-selection-change</code>, <code>viewer-camera-change</code>, <code>viewer-animation-change</code>, <code>viewer-material-change</code>, <code>viewer-environment-change</code>, and <code>viewer-state-export</code>.</li>
    <li><strong>Built-in UI</strong>: drag-and-drop loading, direct mesh picking, material edits, screenshot export, recording, and JSON state copy/import for reproducing the same view later.</li>
</ul>
